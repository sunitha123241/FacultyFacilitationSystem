...
def register():
        ...
        if request.method == 'POST':
            ...
            #c = mysql.connection.cursor()
            c= mysql.connection.cursor()
            c.execute('''SELECT * FROM user1 WHERE id = %s''',(id,))
            x=c.fetchall()
            count = 0
            for k in x:
                count=count+1
            if count > 0:
                ...
                c.execute('''INSERT INTO user1 (id, psw) VALUES (%s, %s)''',(id, psw))
                c.connection.commit()
                c.close()
                #conn.close()
                gc.collect()
...
@app.route('/viewfaulty',methods=['GET','POST'])
def viewfaculty():
    username = session.get('username',None)
    l = []
    cse  = []
...
def casreset():
    ...
    if flag == False:
        ...
        x.close()
        gc.collect()
        return render_template('casreset.html',l=l,y=y,username=username)
...
def conference():
    ...
    if request.method == 'POST':
        ...
        if title == '':
            ...
            s = "insert into conference(id,title,names,barea,ctitle,ind,cdate,venue,jname,issn,volume,issue,page) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            a = (username,title,names,barea,ctitle,ind,(cdate.strftime('%Y-%m-%d')),venue,jname,issn,volume,issue,page)
            x = mysql.connection.cursor()
            x.execute(s,a)
            x.connection.commit()
            x.close()
...
def journal():
    ...
    if request.method == 'POST':
        ...
        if title == '':
            ...
            cdate=datetime.strptime(cdate,"%Y-%m-%d")
            s = "insert into journal(id,title,names,barea,ind,cdate,jname,issn,volume,issue,page) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            a = (username,title,names,barea,ind,(cdate.strftime('%Y-%m-%d')),jname,issn,volume,issue,page)
            x = mysql.connection.cursor()
            x.execute(s,a)
            x.connection.commit()
            x.close()
...
def addfdp():
    ...
    if request.method == 'POST':
        ...
        daycount = daycount + 1
        x = mysql.connection.cursor()
        x.execute('insert into fdp(id,fdptitle,fromdate,todate,venuetype,venue,days) values(%s,%s,%s,%s,%s,%s,%s)',(username,fdptitle,fromdate.strftime('%Y-%m-%d'),todate.strftime('%Y-%m-%d'),venuetype,venue,int(daycount)))
        x.connection.commit()
        x.close()
        gc.collect()
